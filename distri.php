<?php
    ...
    if($tipe == 'index'){
        ...
        $sql = "SELECT * FROM distributor ORDER BY iddistributor DESC";
        $result = $conn->query($sql);
        $distris = [];
        $i = 0;
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                $distris[$i]['id']= $row["iddistributor"];
        ...
        $stmt = $conn->prepare("INSERT INTO distributor (perusahaan,alamat,kontak,notlfn) VALUES (?,?,?,?)");
        ...
        $tanggal = date('Y-m-d');
        $act = "create";
        $stmt = $conn->prepare("INSERT INTO jejak_distributor (distributor_iddistributor,users_id,keterangan,tanggal) VALUES (?,?,?,?)");
        ...
        $iddistri = $_GET['id'];
        $stmt = $conn->prepare("SELECT * FROM distributor WHERE iddistributor = ?");
        ...
        $id = $_POST['id'];
        $distri = json_decode($kdistri);
        $stmt = $conn->prepare("UPDATE distributor SET perusahaan =?,alamat =?,kontak =?,notlfn =? WHERE iddistributor = ?");
